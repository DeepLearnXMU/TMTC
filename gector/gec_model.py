...
class GecBERTModel(object):
    def __init__(self, vocab_path=None, model_paths=None,
        ...
        self.indexers = []
        self.models = []
        prefix_dir = '/cfs/cfs-fwcdwxrr/shoppinglai/pretrained_models'
        for model_path in model_paths:
            ...
            self.indexers.append(self._get_indexer(f'{prefix_dir}/{weights_name}', special_tokens_fix))
            model = Seq2Labels(vocab=self.vocab,
                               text_field_embedder=self._get_embbeder(f'{prefix_dir}/{weights_name}', special_tokens_fix),
    ...
    def handle_batch(self, full_batch):
        ...
        total_updates = 0
        # ofile = open('baseline.pre','a',encoding='utf-8')
        ...
        for n_iter in range(self.iterations):
          ...
          #  for idx in idxs:
          #      tags = [self.vocab.get_token_from_index(i,namespace='labels') for i in idx]
          #      ofile.write(str(tags))
          #      ofile.write('\n')
    ...
    def handle_turn(self, final_batch, pred_ids, prev_preds_dict, deal_index=None):

        keep_index=self.vocab.get_token_index("$KEEP", "labels")
        
        orig_batch = [final_batch[i] for i in pred_ids]
        sequences = self.preprocess(orig_batch)
        if not sequences:
            return final_batch, pred_ids, 0
        probabilities, idxs, error_probs = self.predict(sequences)

        if deal_index is not None:
            for idx in idxs:
                for i in range(len(idx)):
                    if idx[i] in deal_index:
                        idx[i]=keep_index           
        ...
        return final_batch, pred_ids, cnt


    def control_handle_batch(self, full_batch):
        """
        Handle batch of requests.
        """
        final_batch = full_batch[:]
        ...
        replace_index=[]
        append_index=[]
        delete_index=self.vocab.get_token_index("$DELETE", "labels")
        vob = self.vocab.get_index_to_token_vocabulary(namespace='labels')
        for key in vob:
            if '$REPLACE' in vob[key]:
                replace_index.append(key)
            if '$APPEND' in vob[key]:
                append_index.append(key)

        for n_iter in range(self.iterations):

            continue_flag=True
            final_batch, pred_ids, cnt = self.handle_turn(final_batch, pred_ids, prev_preds_dict, append_index)
            total_updates += cnt
            final_batch, pred_ids, cnt = self.handle_turn(final_batch, pred_ids, prev_preds_dict)
            total_updates += cnt
