...
class Trainer(TrainerBase):
    ...
    def padding_mask(self, mask, length=None):
        if length :
            max_len = length
        else:
            max_len = max([len(x) for x in mask])
        for x in mask:
            for t in range(max_len-len(x)):
                x.append(0.)
        return torch.tensor(mask).float().cuda()

    def batch_loss(self, batch_group: List[TensorDict], for_training: bool, stage=3) -> torch.Tensor:
        """
        ...
        if self._multiple_gpu:
            ...
            if stage==1:
                if batch["metadata"] is not None:
                    stage1_mask = [x["mask"] for x in batch["metadata"]]
                
                output_dict = self.model(tokens=batch["tokens"],
                                         labels=batch["mask_labels"],
                                         d_tags=batch["mask_d_tags"],
                                         l_mask=self.padding_mask(stage1_mask,batch["tokens"]["mask"].size(1)),
                                         metadata=batch["metadata"])
            elif stage==2:
                output_dict = self.model(tokens=batch["better_tokens"],
                                         labels=batch["gold_labels"],
                                         d_tags=batch["gold_d_tags"],
                                         metadata=batch["better_metadata"])
            else:
                output_dict = self.model(tokens=batch["tokens"],
                                         labels=batch["labels"],
                                         d_tags=batch["d_tags"],
    ...
    def _train_turn(self, batch_group: List[TensorDict], stage=3)  -> torch.Tensor:
        try:
            loss = self.batch_loss(batch_group, for_training=True, stage=stage) / iter_len
            ...
            for x in batch_group:
                ...
                for elem in ['mask_labels', 'mask_d_tags']:
                    tt = x[elem]
                    print(
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch_group in train_generator_tqdm:
            ...
            # stage 1
            loss_1 = self._train_turn(batch_group, stage=1)
            loss_1.backward()
            
            # stage 2
            loss_2 = self._train_turn(batch_group, stage=2)
            loss_2.backward()
           
            # stage 3
            loss_3 = self._train_turn(batch_group, stage=3)
            train_loss += loss_3.item() * iter_len
            loss_3.backward()
            
            del batch_group, loss_1, loss_2, loss_3
    ...
    def _validation_loss(self) -> Tuple[float, int]:
        ...
        for batch_group in val_generator_tqdm:
            ...
            loss = self.batch_loss(batch_group, for_training=False, stage=3)
            if loss is not None:
